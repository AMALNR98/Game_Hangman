...
def test_mask_word_no_guesses():
    for i in ["police", "elephant", "it", "foo"]:
        assert hangman.masked_word(i, []) == len(i)*"-"


def test_mask_word_invalid_guesses():
    for i in ["police", "elephant", "it", "foo", "cannon"]:
        assert hangman.masked_word(i, ["x", "q"]) == len(i)*"-"


def test_mask_word_single_guesses():
    assert hangman.masked_word('police', ['e']) == '-----e'
    assert hangman.masked_word('police', ['a', 'i', 'p']) == 'p--i--'


def test_mask_word_multiple_guesses():
    assert hangman.masked_word('cannon', ['n']) == '--nn-n'


def test_mask_word_with_no_single_gusses():
    assert hangman.masked_word('police', ['z']) == '------'
...
def test_good_guess_input():
    ...
    turns_left = 5
    new_letter = "l"
    assert hangman.turns(new_letter, guesses, turns_left, secret_word) == (
        turns_left, guesses+['l', ], hangman.CORRECT_LETTER)
...
def test_game_lost():
    ...
    new_letter = "p"
    assert hangman.turns(new_letter, guesses, turns_left, secret_word) == (
        turns_left, guesses, hangman.GAME_LOST)
...
def test_game_won():
    ...
    new_letter = "d"
    assert hangman.turns(new_letter, guesses, turns_left, secret_word) == (
        turns_left, guesses, hangman.GAME_WON)
...
def test_bad_guess_input():
    ...
    new_letter = "y"
    assert hangman.turns(new_letter, guesses, turns_left, secret_word) == (
        turns_left-1, guesses + [new_letter], hangman.WRONG_LETTER)
...
def test_already_guessed_letter():
    ...
    new_letter = "r"
    assert hangman.turns(new_letter, guesses, turns_left, secret_word) == (
        turns_left, guesses, hangman.ALREADY_GUESSED_LETTER)
...
def test_display_board_when_won():
    ...
    result_for_display = """
        ...
        """
    assert hangman.display_board(
        turns_left, secret_word, guesses, result) == result_for_display
